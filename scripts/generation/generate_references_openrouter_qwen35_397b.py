#!/usr/bin/env python3
"""
Generate references from Qwen3.5 397B-A17B (17B active / 397B total MoE) via OpenRouter.
No-geography prompt (topic only). Temperature 0. Resume-safe.

Same 17B active as Llama 4 Scout (109B total) and Maverick (400B total).
Tests whether total params matter when active is held constant.

Usage:
    export OPENROUTER_API_KEY=your_key_here
    python generate_references_openrouter_qwen35_397b.py
"""
...
def call_api(prompt, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if text is None:
                print(f"\n    API returned None content", end=" ", flush=True)
                if attempt < max_retries - 1:
                    time.sleep(10)
                    continue
                return None
            # Strip thinking tags if present (Qwen3.5 may use them)
