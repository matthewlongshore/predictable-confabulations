#!/usr/bin/env python3
"""
Generate references from Hermes 4 405B (reasoning model, Llama 3.1 405B base) via OpenRouter.
No-geography prompt (topic only). Temperature not set (thinking models don't accept it).
Resume-safe.

Same base as Hermes 3 but WITH reasoning — isolates CoT effect on factual recall.

Usage:
    export OPENROUTER_API_KEY=your_key_here
    python generate_references_openrouter_hermes4_405b.py
"""
...
def call_api(prompt, max_retries=5):
    ...
    payload = {
        ...
        # No max_tokens — let reasoning run freely
    }
...
def main():
    ...
    print(f"No temperature (thinking model)")
    print(f"{'='*60}")
